...
export default (props) => {
  return (
    <Template>
      <h1>
        Step 2: Application Timestamp
      </h1>

      <p>
        One way to solve the problem is by using an initializer to store a timestamp of when the application
        was loaded, and then fetching tweets relative to that timestamp.
      </p>

      <p>
        Here's the initializer, called <code>timestamp</code>.
      </p>
      ...
      export default function() {
        // lore.timestamp = moment().toISOString();
        lore.timestamp = new Date().toISOString();
      };
      `}/>

      <p>
        Then update <code>Feed</code> to use it like this:
      </p>
