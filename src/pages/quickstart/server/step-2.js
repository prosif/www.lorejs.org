...
export default (props) => {
  return (
    <Template>
      ...
      <p>
        To fix our app, open up <code>config/connections.js</code>. At the bottom of the file, below the <code>apiRoot</code> you set earlier, you
        will see a section labeled <code>collections</code>. Collections in Lore are the abstraction tier responsible for communicating
        with API endpoints that return an array of resources, such as our <code>/tweets</code> endpoint.
      </p>
      ...
      <p>
        It's commented out because it reflects the default behavior of the framework. When the <code>/tweets</code> endpoint returns data,
        that data is passed to the parse function. This function then has an opportunity to modify the data (referred to as
        <code>attributes</code>) and is expected to return an array of that resource.
      </p>

      <p>
        The default behavior is to return whatever the function was provided. And this worked just fine for our mock API,
        because the original API response <em>was</em> an array of tweets. But now the array we want is embedded inside the <code>data</code>
      ...
      <p>
        The difference is that our <code>userId</code> property has been renamed to <code>user</code>. To solve this problem we <em>could</em> rename every
        reference of <code>userId</code> in our application to <code>user</code>, but Lore provides an alternative way to quickly resolve breaking
        API changes like this. Instead of modifying our components, we're simply going to add the missing <code>user</code> field to
        all tweets.
      </p>

      <p>
        Models and Collections in Lore both have a <code>parse</code> method. The <code>parse</code> method on Models is responsible for parsing a
        single resource and producing the final set of attributes that will be used by the components in your application.
      </p>

      <p>
        While <code>config/collections.js</code> <em>does</em> contain a parse method for <code>models</code>, that method will
        affect <em>all</em> models in the application, which isn't what we want as this issue only affects
        the <code>tweet</code> models.
      </p>
      ...
      <p>
        Instead of modifying all the components, we're simply going to create the missing <code>userId</code> property and copy over the
        value from the <code>user</code> property.
      </p>
      ...
      <blockquote>
        <p>
          ...
          sends it back to the server. If you need to modify a response data sending it to the server you can use the <code>sync</code>
          method.
        </p>
      </blockquote>
      ...
      <p>
        ...
        network tab you'll see the call to <code>/user</code> returns a <code>401 Unauthorized</code>. We'll fix that
