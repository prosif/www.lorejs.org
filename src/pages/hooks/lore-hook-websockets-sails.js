...
export default (props) => {
  return (
    <Template>
      ...
      <p>
        Source code for this hook can be found on GitHub <Link to="https://github.com/lore/lore/tree/master/packages/lore-hook-websockets-sails">at this link</Link>.
      </p>
      ...
      <p>
        ...
        library and implements the interface for Sails.
      </p>
      ...
      <p>
        ...
        you need to install both the <code>socket.io-client</code> <em>and</em> the <code>sails.io.js</code> package (that takes the socket.io client as
        an argument). You also need to set the URL <em>immediately</em> after creating this object, as it will try to call out to
        the server. Alternatively, you can set <code>io.sails.url.authConnect = false</code>. I'm still playing with this implementation
        a bit, but am leaning towards that solution (so that it doesn't make ANY server calls until you tell it to).
      </p>

      <p>
        The other thing to point out is the <code>dispatch</code> implementation. For the most part Sails used a message structure
        consisting of <code>verb</code> and <code>data</code> fields, but sometimes replaces <code>data</code> with <code>previous</code> when it comes to updated and
