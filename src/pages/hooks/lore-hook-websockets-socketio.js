  ...
  return (
    <Template>
      ...
      <ol>
        ...
        <li>
          Provides a set of methods and dispatchers that can be used by default with any model in the application (using some
      ...
      <p>
        Implementing support for socket.io is fairly straight forward. The implementation just needs to know the <code>serverUrl</code>
        (which is the root URL for the socket.io server), the <code>namespace</code> (if your server uses one) and the <code>event</code> that will
      ...
      <p>
        ...
        for data is within the <code>componentDidMount</code> method of the <code>Master</code> component, like so:
      </p>
      ...
      <p>
        Calling <code>lore.websockets.post.connect()</code> will cause the websocket instance to connect with the server. Calling
        <code>lore.websockets.post.subscribe()</code> will cause it to listen for the event it was configured for, such as events
        called <code>post</code> that contain data about Post resources that have been created, updated or deleted by other users.
      </p>
    </Template>
  )
};
